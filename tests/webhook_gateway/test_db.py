...
async def test_create_and_get_endpoint(db):
    ep = EndpointCreate(
        name="GitHub CI", source_type=SourceType.GITHUB, secret="s3cret"
    )
...
async def test_update_endpoint(db):
    ep = await db.create_endpoint(
        EndpointCreate(name="Old", source_type=SourceType.GENERIC)
    )
...
async def test_delete_endpoint(db):
    ep = await db.create_endpoint(
        EndpointCreate(name="ToDelete", source_type=SourceType.GENERIC)
    )
...
async def test_log_webhook_and_list(db):
    ep = await db.create_endpoint(
        EndpointCreate(name="EP", source_type=SourceType.GITHUB)
    )
...
async def test_list_webhooks_pagination(db):
    ep = await db.create_endpoint(
        EndpointCreate(name="EP", source_type=SourceType.GENERIC)
    )
    for i in range(5):
        await db.log_webhook(
            endpoint_id=ep.id,
